...
def small_roots(f, bounds, m=1, d=None, algorithm='groebner', lattice_reduction=None, verbose=False):
    ...
    if lattice_reduction:
        B = lattice_reduction(B.dense_matrix(algorithm='NTL:LLL_XD'))
    else:
    ...
    if algorithm == 'groebner':
        ...
        jacobian_timer = cputime()
        roots = solve_system_with_jacobian(H, f, bounds)
        verbose(f'Solving system with Jacobian took {cputime(jacobian_timer):.3f}s')
        if not roots:
            return []
